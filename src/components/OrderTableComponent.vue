  ...
  <div>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <table class="w-full text-xs sm:text-sm text-left text-gray-500" aria-label="Order Table">
        ...
        <tbody>
          <template
            v-if="props.orderHistory.results.length > 0"
            v-for="(order, index) in props.orderHistory.results"
            :key="index"
          >
          ...
          <template v-else>
            <tr class="bg-white border border-line">
              <td colspan="6" class="text-center text-purple p-3">
                There was in issue loading your orders
              </td>
            </tr>
          </template>
        </tbody>
