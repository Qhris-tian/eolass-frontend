<template>
    ...
    <div v-if="isUpdateAuctionModalOpen" class="modal-mask fixed inset-0 bg-[rgba(0,0,0,0.6)] z-50 grid place-items-center">
        <div class="modal-container bg-white p-4 w-[80vw] max-w-[700px]">
            <header class="">
                <div class="flex justify-between font-medium">
                    <div class="w-full flex justify-center text-center">
                        Update Auction for<br>
                        {{ auctionData?.node.product.name }}
                    </div>
                    <div class="">
                        <img src="@/assets/close-icon.svg" @click="$emit('closeUpdateAuctionModal')" class="cursor-pointer" width="15" height="15" alt="">
                    </div>
                </div>
            </header>
            <div class="flex justify-center">
                <div class="bg-[#F4F7FD] w-[50%] mt-5 p-2 text-center font-medium text-[#FDBF45]">
                    <span class="">
                        Price Update Fee: {{ updateFee.currency == 'USD' ? '$': updateFee.currency }} {{ updateFee.amount }}
                    </span>
                </div>
            </div>
                                ...
                                <div class="mb-4 px-2 w-full md:w-6/12">
                                    ...
                                    <div class="flex flex-row justify-between items-center">
                                        <div class="mb-4 basis-1/4">
                                            <select id="currency" class="w-full border py-2 rounded focus:border-blue-500 focus:shadow-outline outline-none">
                                                <option value="EUR">EUR</option>
                                                <option value="USD">USD</option>
                                            </select>
                                        </div>
                                        <div class="mb-4 px-2 basis-3/4">
                                            <input id="amount" required class="w-full border px-4 py-2 rounded focus:border-blue-500 focus:shadow-outline outline-none" type="text" placeholder="Amount" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-wrap w-full items-center">
                                    <div v-for="(item, index) in removedKeys" :key="index" class="w-1/3 p-1">
                                        <div class="bg-[#EA5555] rounded text-white flex justify-between p-1">
                                            <span class="">{{ item }}</span>
                                            <img src="@/assets/undo-icon.svg" @click="undoKeyRemoval(index)" class="cursor-pointer hover:w-[20px] transition-all" width="15" height="10" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
