  ...
  <div class="w-450 container mx-auto my-5">
      ...
      <div class="relative my-10">
        <form class="w-2/3" @submit.prevent="searchInventory">
          <div class="flex items-center bg-purple-light border border-purple/50 rounded-full shadow-sm">
            <svg-component name="search" class="text-white w-7 absolute ml-5" />

            <div class="w-full rounded-full">
              <input v-model="needle" type="text" aria-label="Search catalog" placeholder="Search inventory"
                autocomplete="off"
                class="flex bg-transparent h-[40px] md:h-[60px] w-[100%] placeholder:text-white indent-[55px] heading-xl text-white border-0 focus:outline-none relative z-10" />
            </div>
        ...
        <div class="absolute z-10 bg-white rounded-b-lg shadow-md mt-2 w-2/3">
          <ul v-if="showSearch">
            <li v-for="(item, index) in searchedItems" :key="index" @click="selectProduct(item)"
    ...
    <div class="flex flex-wrap mt-10">
      <div class="w-full md:max-w-[45%] px-3 order-2 md:order-1">
        ...
        <ul>
          <li class="border p-3 flex rounded-t-md justify-between items-center border-line"
            v-for="(product, index) in availableInventory" :key="index">
      ...
      <div class="w-full md:max-w-[55%] px-3 order-1 md:order-2">
        <p class="heading-lg">{{ selectedProduct?.title ? selectedProduct?.title : 'Product' }} Keys</p>
        <ul v-if="productCards.length > 0">
          <li v-for="(card, index) in productCards" :key="index">
            <div class="flex items-center mr-3">
              <input @input="updateAuctionKeys" :id="card.card_number" type="checkbox"
                class="w-4 h-4 bg-line border-line rounded-md accent-purple" :value="card.card_number" />
              <label :for="card.card_number" class="ml-2 body-text">{{ card.card_number }}</label>
            </div>
          </li>
        </ul>
        <p v-else class="mt-2">
          {{ selectedProduct?.title ? 'Product has no keys associated with it.' : 'Choose product to show keys' }}
        </p>
      </div>
    </div>
  </div>
...
const emit = defineEmits(['update-auction-keys'])

const availableInventory = ref<Array<Product>>([])
onMounted(() => {
  axios.get('/api/v1/inventory/', { params: { limit: 5 } }).then(({ data }) => {
